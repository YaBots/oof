...
services.UserInputService.TextBoxFocused:Connect(function(textBox)
    if textBox.Name == 'ChatBar' then
        focus = true
    end
end)
services.UserInputService.TextBoxFocusReleased:Connect(function(textBox)
    if textBox.Name == 'ChatBar' then
        focus = false
    end
end)
...
local themes = {
    ...
    Midnight = {
        ["Accent"] = Color3.fromRGB(229, 136, 154),
...
function library.createslider(min, max, parent, text, default, float, flag, callback)
    ...
    local function set(value)
        ...
        valuetext.Position = UDim2.new(sizeX, -7, 1, -5)
...
function library:Load(options)
    ...
    local windowtypes = utility.table({tabtoggles = {}, tabtoggleoutlines = {}, tabs = {}, tabtoggletitles = {}, tabselected = {}, count = 0}, true)
    ...
    function windowtypes:Tab(name)
        ...
        local tabselect = utility.create("Square", {
            Filled = true,
            Thickness = 0,
            ...
            Visible = false,
            ZIndex = 10,
            Theme = #self.tabtoggles == 1 and "Accent" or "Accent"
        })

        table.insert(self.tabtoggleoutlines, outline)
        table.insert(self.tabtoggles, tabtoggle)
        table.insert(self.tabselected, tabselect)

        for i, v in next, self.tabtoggles do
            v.Size = UDim2.new(1 / #self.tabtoggles, i == 1 and 1 or i == #self.tabtoggles and -2 or -1, 2, 0)
        ...
        tabtoggle.MouseButton1Click:Connect(function()
            ...
            for _, obj in next, self.tabselected do
                if obj ~= tabselect then
                    obj.Visible = false
                end
            end
        ...
        function tabtypes:Section(options)
                ...
                function toggletypes:Keybind(options)
                    ...
                    local newcallback = function(key, fromsetting)
                        if not fromsetting then
                            set(not toggled and focus == false)
                        end
            ...
            function sectiontypes:Slider(options)
                ...
                section.Size = UDim2.new(1, 0, 0, sectioncontent.AbsoluteContentSize + 35)
